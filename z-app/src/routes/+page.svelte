...
<script lang="ts">
	import { themeManager, THEMES, getThemeDisplayName } from '$lib/stores/theme';
	...
	// Reactive values
	const currentAppliedTheme = $derived(g.theme.applied);
	const isCurrentlyDark = $derived($currentAppliedTheme === THEMES.DARK);
	...
	const simulateLoading = async () => {
		...
		await coreo.delay(3000, async () => {
			isLoading = false;
			message = `Operation completed in ${currentAppliedTheme} mode!`;
			await coreo.delay(1000);
			message = '';
		});
	};
...
<div class="space-y-6">
	...
	<div style="background: #f0f0f0; padding: 1rem; border-radius: 8px; margin: 1rem 0;">
		...
		<button
			onclick={() => themeManager.setTheme(THEMES.DARK)}
		...
		<p style="margin: 0.5rem 0; color: #666; font-size: 14px;">
			Current: {g.theme.current} | Applied: {$currentAppliedTheme}
		</p>
	</div>
	...
	<div class="card">
		...
		<div class="grid-2 gap-4">
			<div>
				...
				<div class="badge badge-primary">{getThemeDisplayName(g.theme.value)}</div>
			</div>
			<div>
				<p class="text-small mb-2">Applied Theme:</p>
				<div
					class="badge {$currentAppliedTheme === THEMES.DARK
				...
				<div class="flex flex-wrap gap-2">
					{#each Object.values(THEMES) as themeOption}
						<button
							...
							onclick={() => themeManager.setTheme(themeOption)}
						>
